...
var Component = React.createClass({
    ...
    componentWillUnmount: function () {
        ...
        if (Object.keys(listeners.tooltips).length) {
            Object.keys(listeners.tooltips).forEach(function (key) {
                document.querySelector(key).removeEventListener('click', listeners.tooltips[key]);
    ...
    componentDidUpdate: function (prevProps, prevState) {
        if ((this.state.tooltip || (this.state.play && this.props.steps[this.state.index])) && this.state.xPos < 0) {
            this._calcPlacement();
        }
    ...
    addTooltip: function (data) {
        ...
        if (parseData.length) {
            data = parseData[0];
            el = document.querySelector(data.trigger || data.selector);
        }

        listeners.tooltips[data.trigger || data.selector] = this._onClickTrigger;

        el.dataset.tooltip = JSON.stringify(data);
        el.addEventListener('click', listeners.tooltips[data.trigger || data.selector]);
    },
