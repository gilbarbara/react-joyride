...
var Mixin = {
    ...
    _renderCurrentStep: function () {
        var state       = this.state,
            ...
            buttonText  = state.joyrideType === 'guided' ? (hasNextStep ? (state.joyrideLocale.next + (state.joyrideShowStepsCount ? ' ' + (state.joyrideCurrentIndex + 1) + '/' + joyride.steps.length : '')) : state.joyrideLocale.last) : state.joyrideLocale.close,
