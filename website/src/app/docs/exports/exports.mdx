...
Objects you can use to compare against values in [<Code color="primary">EventData</Code>](./events#eventdata). All values are typed `as const`.
...
| Constant | Value |
|----------|-------|
| **ORIGIN.BUTTON_BACK** | `'button_back'` |
...
The `origin` is `null` when the action was triggered programmatically via controls rather than by a built-in UI element.
...
An object containing the default values for all [<Code color="primary">Options</Code>](./props/options) fields (colors, sizes, behavior flags, timing, etc.).

### defaultLocale

An object containing the default [<Code color="primary">Locale</Code>](./props#locale) strings (button labels and aria-labels).
