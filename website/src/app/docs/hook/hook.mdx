...
<Callout type="tip">
  <p>The handler receives [`EventData`](./events#eventdata) and [`Controls`](#controls) as arguments — the same as `onEvent`. Since `data` contains the full tour state (`index`, `action`, `status`, etc.), you rarely need external state in the handler.</p>
</Callout>
